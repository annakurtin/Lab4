...
```{r warning=FALSE}
# Bringing in a raster from Arc and assigning the categories to the levels 
landcover<-rast(here::here("Data","landcover"))
# always better to turn ArcGIS raster into a .tiff file or some other format 
image(landcover, col=rainbow(16))
landcover
# 30x30 min is landsat data 
cats(landcover)
# not a categorical raster
head(values(landcover))
# everything is 10 - no attributes table 
levels(landcover)
...
# need to create a layer file that didn't export over from ArcGIS
ID <- 0:16
...
cats(landcover)
## look at remote sensing for large area habitat mapping (paper)
```
...
# save what we did as a .tif file to make it easier to work with
...
```{r, eval = FALSE}
# method is nearest neighbor resampling 
## gets the majority category of the cells near the cell specified 
## can change how many "neighbors" you go out to; the default is one grid cell
landcover2<-resample(landcover16, mask.raster, method="near") 
ext(landcover2)
plot(landcover2)
# values haven't changed but extent has changed, now when you make a raster stack you will be able to get them all to align 
```
...
#summary(wolfkde)
#table(wolfkde$used, wolfkde$pack)
# check notes for design matrix for categorical variables 
```
...
# Running the models from last week - but we are only interested in the coefficients from these models
# models are an S3 object with 30 values of it
```
...
```{r}
levels(wolfkde$landcov.f) ## see, all we have is landcover code

# see how do to this with pivot_wider
wolfkde %>% pivot_wider(names_from=habitatType)
...
```{r}
# delete clouds from the data frame 
wolfkde2 <- wolfkde[wolfkde$landcover16 != 11, ]
wolfkde3 <-wolfkde2[wolfkde2$landcover16 != 0, ]
# these will be blanks in the land cover data - you can't make any inference to the areas under the clouds 
...
# reorder the data into a matrix
```
...
# probability of use is prob of use divided by 413
...
landcovSelection2
# now have proportions of use vs available 
```
...
# axis is landcover
# y axis is selectivity coefficient
## untransformed - values below one are used less often than expected
## values above one are being selected for 
```
...
# here is the plot of the transformed selectivity ratio on the y axis 
```
...
# adehabitat will calculate either proportions or absolute numbers
...
## in this case, elk use by four categories is significant - elk are using categories of resources different than if they were using them randomly 
## wi is not lon transformed selectivity ratios
## bi interpreted as true probability of elk using those locations
## ex for any random telemetry locations, 42% of them would use cover of 26-75%

## plot the values of the selection ratios
plot(wiRatio)

# calculates confidence intervals and other things for you 
```

*Homework:* take landcovSelection2
Try using both raw numbers and proportions - walk through it in the adehabitat package using widesI()
...
ANOVA only tells you if there's a difference between one of the categories, not which one. Contrasts are what is generated when you go through and determine which ones are significantly different. 
```{r}
...
# we see that there are 15 categories and 16 land types
# open conifer has zero for all of them, meaning that it is the reference variable 
```
...
In this case, we are letting the animals (wolves) tell us which categories to condense into one category
```{r}
...
Need to do interaction expansion to tell R that this is a category
...
# in this output, the coefficient of I(landcov etc.) is positive and everything else is lumped into the intercept
## tells you that open conifer is being selected for more than anything else 
```
...
## this gives you the probability of wolf use in open conifer 
...
This is how you take a categorical variable and put every category into it 
```{r}
# Full model
...
```{r}
# take the values estimated and look at the probability in the logistic regression equation
## it's trying to tell you the probability of something when it wasn't used at all, which is difficult to do 
exp(-0.974 - 15.655*1)/(1+exp(-0.974 - 15.655*1)) 
```
...
When your standard error is blowing up, it's an indication that you don't have enough data to say anything about this covariate. 
...
Coefficients for moderate and deciduous are different between the model with an intercept and without an intercept 
...
contrasts(wolfkde3$landcov.f) = contr.treatment(13, base = 10) # this is how you manually set the reference category 
attributes(wolfkde3$landcov.f)
# and note that rock-ice now is 0. 

# set the rock intercept model 
...
Kicking out variables that aren't statistically significant isn't valid in a categorical logistic regression
...
### refitting model manually dropping Decid and Regen - where do they now go?
...
## by dropping them from the model, they are now in the intercept (everything else not specified in the model)

## use AIC to determine the best model
## AIC doesn't tell you the best intercept to use 
```
...
Estimates of uncertainty for log reg model come from logit link
Est from manleys hand calc long selection ratio come from habitat ade package

Point is that the intercepts are the same between the two of them. 
take landcovSelection and compare selection ratios to B from log reg


```{r}
# need to recode the contrast 


# need to run the model again 
no.int.topmodel = glm(used~openConif + closedConif+modConif+mixed+herb+rock+ice+burn+alpine, data = wolfkde3, family - binomial(logit))
